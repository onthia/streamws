...
function initAsClient(address, options) {
    ...
    if (
        options.value.protocolVersion != 8 &&
        options.value.protocolVersion != 13
    ) {
    ...
    // begin handshake
    var key = new Buffer(
        options.value.protocolVersion + '-' + Date.now()
    ).toString('base64');
    ...
    if (options.value.origin) {
        if (options.value.protocolVersion < 13)
            requestOptions.headers['Sec-WebSocket-Origin'] =
    ...
    (isNodeV4 ? agent : req).once(
        ...
        function (res, socket, upgradeHead) {
            ...
            if (
                typeof serverKey == 'undefined' ||
                serverKey !== expectedServerKey
            ) {
...
function establishConnection(ReceiverClass, SenderClass, socket, upgradeHead) {
    ...
    self._receiver.onping = function (data, flags) {
        ...
        self.pong(
            data,
            { mask: !self._isServer, binary: flags.binary === true },
            true
        );
